 ...
 * @author Chingo
 */
class XMLPlacementDeserializer<T extends IPlacement> {
    
    private Method load;
    private XMLDeserializer loader;

    XMLPlacementDeserializer(Method load, XMLDeserializer loader) {
        Preconditions.checkArgument(Modifier.isStatic(load.getModifiers()), "Loader method must be static!");
        this.load = load;
        this.loader = loader;
    }
    
    T deserialize(Element placementElement) {
        try {
            return (T) load.invoke(null, placementElement);
