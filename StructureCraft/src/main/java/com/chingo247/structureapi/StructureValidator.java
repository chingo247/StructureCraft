...
import com.chingo247.structureapi.model.persistent.structure.ConstructionStatus;
...
import com.chingo247.structureapi.model.persistent.owner.OwnerType;
import com.chingo247.structureapi.model.persistent.world.StructureWorldRepository;
...
public class StructureValidator {
    ...
    public StructureValidator(IServer server, ExecutorService executor, GraphDatabaseService graph, IEconomyProvider economyProvider) {
        this.server = server;
        this.graph = graph;
    ...
    public void invalidate() {
        System.out.println("[StructureCraft]: Starting structure invalidation...");
                ...
                if (w != null) {
                    ...
                    if (n.hasProperty(LOCK_DATA)) {
                        ...
                        try {
                            ...
                            Logger.getLogger(StructureValidator.class.getName()).log(Level.SEVERE, ex.getMessage(), ex);
                        }
                    }
                }
            }
    ...
    private void processDeletedAfter(IWorld world, long date) {
        ...
        try (Transaction tx = graph.beginTx()) {
            ...
            if (!structureNodes.isEmpty()) {
                System.out.println("[StructureCraft]: Restoring " + structureNodes.size() + " structures from '" + world.getName() + "' which have been deleted after the last world save ");
            } else {
                System.out.println("[StructureCraft]: Nothing to restore");
                tx.success();
                return;
            }
...
//            }
    ...
    private void processCreatedAfter(IWorld world, long date) {
        ...
        try (Transaction tx = graph.beginTx()) {
            ...
            if (!structureNodes.isEmpty()) {
                System.out.println("[StructureCraft]: Found a total of " + structureNodes.size() + " structures within " + world.getName() + " that are invalid");
                System.out.println("[StructureCraft]: These structures have been placed after the last world save ");
            } else {
                System.out.println("[StructureCraft]: Nothing to invalidate");
                tx.success();
                return;
            }
            ...
            // AUTO REFUND
            if (economy != null) {
                System.out.println("[StructureCraft]: Refunding players which own invalid structures within " + world.getName());
                for (StructureNode sn : structureNodes) {
                    if (sn.getPrice() > 0 && !sn.isAutoremoved()) {
                        ...
                        for (SettlerNode settler : masters) {
                            ...
                            System.out.println("[StructureCraft]: Refunded " + ShopUtil.valueString(pricePerOwner) + " to " + settler.getName()
