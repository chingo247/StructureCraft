...
import com.chingo247.structureapi.io.blockstore.IBlockStoreChunkFactory;
...
public class SafeBlockStoreChunkFactory implements IBlockStoreChunkFactory<ISafeBlockStoreChunk>{
    ...
    @Override
    public ISafeBlockStoreChunk newChunk(@Nullable Tag sectionTag, int x, int z, Vector2D size) {
        ISafeBlockStoreChunk bsc;
        if (sectionTag == null) {
            bsc = new SafeBlockStoreChunk(blockStore, new HashMap<>(), x, z, size);
        } else {
            bsc = new SafeBlockStoreChunk(blockStore, (Map) sectionTag.getValue(), x, z, size);
        }
        return bsc;
    }
