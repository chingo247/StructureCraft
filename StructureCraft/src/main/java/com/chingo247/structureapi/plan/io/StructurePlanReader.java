...
import com.chingo247.structureapi.placement.io.PlacementRegister;
...
import com.chingo247.structureapi.plans.StructurePlan;
import com.chingo247.structureapi.plan.SubstructurePlan;
...
public class StructurePlanReader {
    ...
    private class StructurePlanProcessor extends RecursiveTask<IStructurePlan> {
        ...
        private SubstructurePlan parent;
        ...
        StructurePlanProcessor(File structurePlan, SubstructurePlan parent) {
...
//                } else {
                    StructurePlan plan = new StructurePlan(id, structurePlanFile, placementProcessor.get());
                    plan.setName(name);
                    plan.setPrice(price);
                    plan.setDescription(description);
                    plan.setCategory(category);
...
//        }
    ...
    private class PlacementProcessor extends RecursiveTask<IPlacement> {
        ...
        protected IPlacement compute() {
            try {
                return PlacementRegister.instance().deserialize(new XMLElement(placeableNode.getFile(), placeableNode.getElement()));
