 ...
 *
 * @author Chingo
 * @param <T> The placement type
 */
public class PlacementSaver<T extends IPlacement> {
    
    private Method save;
    private XMLDeserializer saver;

    public PlacementSaver(Method load, XMLDeserializer saver) {
        Preconditions.checkArgument(Modifier.isStatic(load.getModifiers()), "Saver method must be static!");
        this.save = load;
        this.saver = saver;
    }
    
    public void save(T placement, File file) {
        try {
            save.invoke(null, placement);
